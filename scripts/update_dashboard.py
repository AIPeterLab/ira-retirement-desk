...
def fetch_btc_signals():
    ...
    generated=datetime.fromisoformat(source["generated_at"].replace("Z","+00:00")); age=(datetime.now(timezone.utc)-generated.astimezone(timezone.utc)).total_seconds()/3600
...
def simulate_ira(px,qs,ss,bs):
    ...
    for d in days[1:]:
        ...
        values["IRA_BTC"]*=btc_weight*px["BTC-USD"][d]/px["BTC-USD"][prev]+(1-btc_weight)
        prev=d
...
def assert_btc_consistency(payload,source):
    ...
    sleeve=next((x for x in payload["sleeves"] if x["name"]=="IRA_BTC"),None); metric=next((x for x in payload["metrics"] if x["label"]=="BTC strategy"),None)
    if not sleeve or sleeve.get("position")!=expected or sleeve.get("btc_allocation_pct")!=allocation or not metric or metric.get("value")!=expected:raise RuntimeError("IRA BTC output differs from BTC Signal Desk")
def write():
    qqq_source,qs=fetch_qqq_signals(); spy_source,ss=fetch_spy_signals(); btc_source,bs=fetch_btc_signals(); px={s:fetch(s) for s in SYMBOLS}
    ...
    if MODE=="roth":
        ...
        base,market=simulate_ira(px,qs,ss,bs); base={k:round(v,2) for k,v in base.items()}; start_total=sum(IRA_START_VALUES.values()); total=sum(base.values()); growth=sum(base[k] for k in ["IRA_QQQ","IRA_SPY","IRA_BTC"]); specs=[("IRA_QQQ",40,35,45,qpos,"Risk-on" if qpos=="QLD" else "Defensive"),("IRA_SPY",40,35,45,spos,"Risk-off" if spos=="Cash" else "Risk-on"),("IRA_BTC",20,15,25,bpos,"Partial allocation" if 0<btc_allocation<100 else "Risk-on" if btc_allocation==100 else "Risk-off")]; sleeves=[]
        for n,t,lo,hi,p,s in specs:
            w=base[n]/growth*100; sleeve={"name":n,"value":round(base[n],2),"weight":w,"target":t,"drift":w-t,"position":p,"signal":s,"action":status_action(w,t,lo,hi),"note":"Growth sleeve weight"}
            if n=="IRA_BTC":sleeve.update({"btc_allocation_pct":btc_allocation,"cash_allocation_pct":100-btc_allocation,"effective_btc_weight":w*btc_allocation/100,"effective_total_btc_weight":base[n]*btc_allocation/100/total*100,"note":f"{btc_allocation:g}% of this 20% sleeve is in BTC; {100-btc_allocation:g}% is cash"})
